...
function About({ title, description }) {
  ...
  return (
    ...
    // </div>
    // </section>
    <section className="flex items-center justify-center h-fit my-20 p-10 bg-fixed bg-contain bg-left-top custom-img mx-auto rounded-xl max-w-3xl md:mx-auto text-2xl text-slate-700" style={parallax}>
      <div>
          <p className="text-2xl text-white md:text-4xl font-bold text-center">{title}</p>
          {description.map((item, index) => (
            <p key={index} className="text-base text-white text-justify md:text-center text-slate-600 leading-relaxed mt-4">
              {item}
            </p>
          ))}
